...
test.describe("PR #38 UI smoke", () => {
  test("app loads with Claude-style sidebar", async ({ page }) => {
    ...
    await expect(
      page.locator('aside, [class*="sidebar" i]').first(),
    ).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test("chat send shows an in-progress phase, then a result", async ({
    page,
  }) => {
    await page.goto(APP_URL);
  ...
  test("only the chat area scrolls, not the whole viewport", async ({
    page,
  }) => {
    await page.goto(APP_URL);
