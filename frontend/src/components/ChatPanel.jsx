...
function BubbleAvatar({ role, hidden }) {
  if (hidden)
    return (
      <span
        className="bubble-avatar bubble-avatar--spacer"
        aria-hidden="true"
      />
    );
  if (role === "user") {
...
function inferUploadContext(messages) {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    for (const call of msg.tool_calls) {
      ...
      if (
        !docType &&
        call.function.name === "request_template" &&
        args.doc_type
      )
...
export default function ChatPanel({
  ...
  async function sendToBackend(outgoing) {
    ...
    try {
      ...
      for (const msg of newlyAdded) {
        if (msg.role !== "tool" || msg.name !== "propose_delete_client")
          continue;
        try {
          const result = JSON.parse(msg.content);
          if (result.found && result.needs_confirmation)
  ...
  async function handleSend(textOverride) {
    ...
    const outgoing = updated.filter(
      (m) => m.role !== "upload-result" && m.role !== "delete-result",
    );
            ...
            <div className="bubble-row bubble-row--assistant">
              ...
              <div className="bubble bubble--assistant">
                <div className="bubble__text">
                  Hey! What can I help with — checking on a client, grabbing a
                  template, or filing something you've finished? You can also
                  just drop a file in with the 📎 below if that's easier.
                </div>
              </div>
            </div>
          )}
          {(() => {
            ...
            return messages.map((msg, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`bubble-row bubble-row--${msg.role}${grouped ? " bubble-row--grouped" : ""}`}
                >
