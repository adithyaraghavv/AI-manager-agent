...
def test_client_with_no_documents_is_blocked_at_first_phase(rest):
    rest.insert(
        "clients", {"name": "Acme", "created_at": _iso(datetime.now(timezone.utc))}
    )
...
def test_old_activity_on_incomplete_client_is_stale(rest):
    ...
    rest.insert(
        "client_documents",
        {"client_id": client["id"], "doc_type": "MSA", "uploaded_at": _iso(old)},
    )
...
def test_old_activity_on_complete_client_is_not_stale(rest):
    ...
    for doc_type in ("MSA", "SOW", "BRD"):
        rest.insert(
            "client_documents",
            {"client_id": client["id"], "doc_type": doc_type, "uploaded_at": _iso(old)},
        )
