...
def test_embed_document_works_for_any_doc_type_not_just_sow(rest):
    ...
    with patch("app.services.embedding_service.OpenAI") as MockOpenAI:
        MockOpenAI.return_value.embeddings.create.return_value = (
            _mock_embeddings_response([[1.0, 0.0]])
        )
...
def test_embed_document_stores_one_row_per_chunk(rest):
    ...
    with patch("app.services.embedding_service.OpenAI") as MockOpenAI:
        ...
        MockOpenAI.return_value.embeddings.create.return_value = (
            _mock_embeddings_response([[float(i)] * 3 for i in range(len(pieces))])
        )
        count = embed_document(
            rest,
            client_id=1,
...
def test_embed_document_replaces_prior_chunks_for_same_version(rest):
    with patch("app.services.embedding_service.OpenAI") as MockOpenAI:
        ...
        embed_document(
            ...
            doc_type="SOW",
            version_number=1,
            content=b"first upload text",
            extension="txt",
        )

        MockOpenAI.return_value.embeddings.create.return_value = (
            _mock_embeddings_response([[0.0, 1.0]])
        )
        embed_document(
            rest,
            client_id=1,
...
def test_embed_document_returns_zero_on_embedding_api_failure(rest):
    ...
    with patch("app.services.embedding_service.OpenAI") as MockOpenAI:
        MockOpenAI.return_value.embeddings.create.side_effect = RuntimeError(
            "network error"
        )
        count = embed_document(
            rest,
...
def test_search_document_content_ranks_by_similarity(rest):
    ...
    rest.insert(
        "document_chunks",
        {
            ...
            "version_number": 1,
            "chunk_index": 1,
            "content": "unrelated pricing text",
    ...
    with patch("app.services.embedding_service.OpenAI") as MockOpenAI:
        MockOpenAI.return_value.embeddings.create.return_value = (
            _mock_embeddings_response([[1.0, 0.0, 0.0]])
        )
        matches = search_document_content(
            rest, client_id=1, query="what's the termination policy"
        )
...
def test_search_document_content_filters_by_doc_type(rest):
    ...
    with patch("app.services.embedding_service.OpenAI") as MockOpenAI:
        ...
        matches = search_document_content(
            rest, client_id=1, query="anything", doc_type="SOW"
        )
